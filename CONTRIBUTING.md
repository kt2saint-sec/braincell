...
cross-contaminate it. It then builds and checks the wheel and sdist in a
separate no-swap cgroup. Its per-run directory records memory peak, process
count, and cgroup pressure events; any pressure event fails the release check
and retains the artifacts for inspection. It refuses an uncaged invocation, a
non-NVMe workspace, or insufficient free disk space.
